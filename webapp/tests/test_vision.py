...
def test_textdiagnose_unveraendert_kein_vision_key() -> None:
    sink: list = []
    restore = _patch_backend(sink)
    try:
        result = ai.diagnose("Mein Toaster wirft nicht aus")
        assert "device" in result and result["source"] == "ai"
        assert "vision" not in result["diagnosis"], "Text-only darf KEINEN vision-Vermerk haben"
        # Nutzer-Message ist ein reiner String (kein multimodaler Inhalt)
        user_msg = sink[0]["messages"][1]["content"]
        assert isinstance(user_msg, str) and user_msg.startswith("Problembeschreibung:")
    finally:
        restore()


def test_diagnose_mit_extraktion_setzt_vision_marker() -> None:
    sink: list = []
    restore = _patch_backend(sink)
    try:
        ex = {"felder": {"kategorie": {"wert": "Waschmaschine", "erkannt": True}}}
        result = ai.diagnose("pumpt nicht ab", extraktion=ex)
        v = result["diagnosis"].get("vision")
        assert v is not None and v["einbezogen"] is False and v["medienAnzahl"] == 0
        # Bestätigtes Feld ist im Prompt-Text gelandet
        user_msg = sink[0]["messages"][1]["content"]
        assert "Waschmaschine" in user_msg
    finally:
        restore()


def test_diagnose_mit_bild_sendet_multimodal_und_markiert_einbezogen() -> None:
    sink: list = []
    restore = _patch_backend(sink)
    try:
        bild = "data:image/png;base64,QUJD"
        result = ai.diagnose("pumpt nicht ab", bilder=[bild])
        v = result["diagnosis"].get("vision")
        assert v is not None and v["einbezogen"] is True and v["medienAnzahl"] == 1
        # multimodaler Inhalt: Liste mit text + image_url
        content = sink[0]["messages"][1]["content"]
        assert isinstance(content, list)
        typen = [c.get("type") for c in content]
        assert "text" in typen and "image_url" in typen
    finally:
        restore()
...
def test_endpoint_diagnose_ohne_bild_unveraendert() -> None:
    c = app.app.test_client()
    r = c.post("/api/diagnose", json={"text": "Toaster kaputt"})
    assert r.status_code == 503 and r.get_json()["code"] == "no_backend"
