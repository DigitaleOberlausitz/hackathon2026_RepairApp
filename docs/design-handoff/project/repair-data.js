/* repair-data.js — Inhaltsmodell der Repair-App (Deutsch).
   Zwei Kontrast-Szenarien: Toaster (grün, gut selbst machbar) und
   Mikrowelle (rot, gefährlich → Profi). Als window-Global geteilt. */
(function () {
  // Eine Ampel: level 'gut' | 'mittel' | 'stop'
  const G = 'gut', M = 'mittel', S = 'stop';
  ...
  const TOASTER = {
    ...
    confidence: { level: 'hoch', source: 'geprüfte Reparatur-Anleitung', note: 'Häufiges, gut dokumentiertes Problem bei diesem Toaster-Typ.' },
    recommend: 'self',
  ...
  const MIKROWELLE = {
    ...
    confidence: { level: 'mittel', source: 'teils KI-Einschätzung', note: 'Die genaue Ursache ist aus der Ferne nicht sicher — die Gefahreneinschätzung dagegen schon.' },
    recommend: 'pro',
  ...
  window.REPAIR_DEVICES = { toaster: TOASTER, mikrowelle: MIKROWELLE };
  window.REPAIR_LEVELS = { G, M, S };
})();
